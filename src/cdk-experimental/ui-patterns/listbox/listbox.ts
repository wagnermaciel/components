...
export type ListboxInputs<V> = ListInputs<OptionPattern<V>, V> & {
  ...
  combobox: SignalLike<ComboboxPattern<OptionPattern<V>> | undefined>;
};
...
export class ListboxPattern<V> {
  ...
  constructor(readonly inputs: ListboxInputs<V>) {
    ...
    if (this.inputs.combobox()) {
      ...
      this.inputs.element = this.inputs.combobox()!.inputs.element;
    }
  ...
  comboboxActions: ComboboxPopupControls<OptionPattern<V>> = {
