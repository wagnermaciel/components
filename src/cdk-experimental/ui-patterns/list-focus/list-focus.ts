...
export class ListFocus<T extends ListFocusItem> {
  ...
  constructor(args: ListFocusInputs<T> & {navigation: ListNavigation<T>}) {
    this.navigation = args.navigation;
    this.focusStrategy = args.focusStrategy;
  }
  ...
  getActiveDescendant(): Signal<string | null> {
    return computed(() => {
      ...
      return this.navigation.items()[this.navigation.activeIndex()].id();
    });
  }

  /** Returns a signal that keeps track of the tabindex for the list. */
  getListTabindex(): Signal<-1 | 0> {
    return computed(() => (this.focusStrategy() === 'activedescendant' ? 0 : -1));
  }
  ...
  getItemTabindex(item: T): Signal<-1 | 0> {
    return computed(() => {
      ...
      const index = this.navigation.items().indexOf(item);
      return this.navigation.activeIndex() === index ? 0 : -1;
    });
  }
